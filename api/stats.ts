...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  const client = createDbClient(); // do NOT call connect()
  ...
  try {
    ...
    const { rows: todayLogs } = await client.sql`
      SELECT visitor_id, status FROM visitor_logs WHERE check_in >= ${todayEpoch}
    `;
    const { rows: allIn } = await client.sql`
      SELECT count(*) as count FROM visitor_logs WHERE status = ${VisitorStatus.IN}
    `;
    const { rows: eventsCount } = await client.sql`
