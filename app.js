// app.js - Enhanced with Advanced UI & Animations
    ...
    return UI.templates.base("Welcome", `
      ...
      <script>
        ...
        async function loadStats() {
          try {
            const [visitors, checkedIn, events] = await Promise.all([
              fetch('/api/stats/visitors').then(r => r.json()),
              fetch('/api/stats/checked-in').then(r => r.json()),
              fetch('/api/stats/events').then(r => r.json())
            ]);
            
            document.getElementById('visitorsCount').textContent = visitors.count;
            document.getElementById('checkedInCount').textContent = checkedIn.count;
    ...
    res.send(UI.templates.base("Error", `
      <div class="min-h-screen flex items-center justify-center py-12">
        <div class="card p-8 max-w-md w-full mx-4 text-center animate__animated animate__shakeX">
          ...
          <p class="text-gray-600 mb-6">Error during auto check-in/out. Please try again.</p>
...
app.get("/admin", requireAdmin, async (req, res) => {
  ...
  try {
    const rows = await db.all("SELECT * FROM visitors ORDER BY checkIn DESC");
...
app.get("/api/stats/visitors", async (req, res) => {
  const db = await dbPromise;
  try {
    const row = await db.get("SELECT COUNT(*) as count FROM visitors");
...
app.get("/api/stats/checked-in", async (req, res) => {
  const db = await dbPromise;
  try {
    const row = await db.get("SELECT COUNT(*) as count FROM visitors WHERE status = 'IN'");
    res.json({ count: row.count });
  } catch (err) {
    res.json({ count: 0 });
  }
...
app.get("/api/stats/events", async (req, res) => {
  const db = await dbPromise;
  try {
    const row = await db.get("SELECT COUNT(DISTINCT eventName) as count FROM visitors WHERE eventName != 'First Visit' AND eventName != 'Automatic Check-in'");
    res.json({ count: row.count });
  } catch (err) {
    res.json({ count: 0 });
  }
