        ...
        <div className="min-h-[80vh] flex items-center justify-center p-4">
            <Card className="w-full max-w-lg">
                <div className="p-8">
                    ...
                    {!eventName && isEvent && <div className="text-center mb-6"><Spinner /></div>}
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        {isGate && (
                            <>
                                ...
                                <Input id="laptopName" name="laptopName" placeholder="Laptop's Name" value={formData.laptopName} onChange={handleChange} required />
                                <Input id="laptopColor" name="laptopColor" placeholder="Laptop's Colour" value={formData.laptopColor} onChange={handleChange} required />
                                <Input id="serialNumber" name="serialNumber" placeholder="Serial Number" value={formData.serialNumber} onChange={handleChange} required />
                                    ...
                                    {Object.entries(internTypes).map(([key, label]) => (
                                        <div key={key} className="flex items-center">
                                            <input
                                                ...
                                                className="h-4 w-4 text-emerald-600 bg-gray-700 border-gray-600 focus:ring-emerald-500"
                                            />
                                            <label htmlFor={`visitorType-${key}`} className="ml-2 block text-sm text-gray-300">
                                                {label}
                                            </label>
                                        </div>
                                    ))}
                        ...
                        {isEvent && (
                            <>
                                <Input id="organization" name="organization" placeholder="Organization" value={formData.organization} onChange={handleChange} required />
...
const RegistrationSuccess: React.FC<{}> = () => {
    ...
    const message = mode === 'event' 
        ...
        : (mode === 'intern' ? "Your attendance has been successfully recorded. Your device is now remembered for future visits." : "You have been successfully checked in. Your device is now remembered for future visits.");

    const subMessage = mode === 'event'
        ? "No further action is needed. Enjoy the event!"
        : (mode === 'intern' ? "Next time, simply scan the QR code to mark your attendance for the day." : "Next time, simply scan the QR code to automatically check in or out.");
...
const AdminDashboard: React.FC<{ onLogout: () => void; db: MockDb }> = ({ onLogout, db }) => {
    ...
    const [editFormData, setEditFormData] = useState({ ...initialManualEntryState, checkIn: '', checkOut: '' });
    ...
    const handleManualEntrySubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!manualEntryData.name.trim() || !manualEntryData.department.trim() || !manualEntryData.checkIn || !manualEntryData.laptopName.trim() || !manualEntryData.laptopColor.trim() || !manualEntryData.serialNumber.trim()) {
            alert('Please fill out all required fields (Name, Department, Laptop Details, Check-in Time).');
            return;
        }
    ...
    const handleOpenEditModal = (log: VisitorLog) => {
        ...
        setEditFormData({
            ...
            visitorType: log.visitorType || VisitorType.GUEST,
            checkIn: '', // Not used for editing
            checkOut: '', // Not used for editing
        });
    ...
    const handleUpdateLogSubmit = async (e: React.FormEvent) => {
        ...
        if (!editFormData.name.trim() || !editFormData.department.trim() || !editFormData.laptopName.trim() || !editFormData.laptopColor.trim() || !editFormData.serialNumber.trim()) {
        ...
        try {
            await updateVisitorLog(editingLog.id, {
                ...
                visitorType: editFormData.visitorType as VisitorType,
            });
            ...
            showToast("Error: Could not update log.");
        } finally {
    ...
    const filteredLogs = React.useMemo(() => {
        // Only show logs for the main gate
        let tempLogs = logs.filter(log => log.context === 'gate');
    ...
    const filteredInternLogs = React.useMemo(() => {
        let tempLogs = logs.filter(log => log.context === 'intern');
        ...
        if (searchQuery.trim() !== '') {
            ...
            tempLogs = tempLogs.filter(log => {
                return (
                    log.name?.toLowerCase().includes(lowercasedQuery) ||
                    (log.visitorType === VisitorType.CORPER && 'nysc'.includes(lowercasedQuery)) ||
                    log.visitorType?.toLowerCase().includes(lowercasedQuery)
                );
            });
        }
    ...
    const internReportData = useMemo(() => {
        const targetDate = new Date(reportYear, reportMonth);
        const isCurrentMonth = new Date().getMonth() === reportMonth && new Date().getFullYear() === reportYear;
        
        // Calculate days passed in the month for expectation
        // If it's the current month, we calculate based on days passed so far to be fair.
        // If it's a past month, we calculate based on the full month.
        ...
        // Requirement: 3 times a week.
        // Approximate: daysPassed / 7 * 3
        const expectedAttendance = Math.floor((daysPassed / 7) * 3);
        const finalExpected = Math.max(1, expectedAttendance); // Avoid division by zero, at least 1 expected if some time passed

        const internLogs = logs.filter(log => {
            if (log.context !== 'intern') return false;
            const logDate = new Date(log.checkIn);
            return logDate.getMonth() === reportMonth && logDate.getFullYear() === reportYear;
        });
        ...
        internLogs.forEach(log => {
            if (!report.has(log.visitorId)) {
                report.set(log.visitorId, {
            ...
            report.get(log.visitorId)!.attendedDates.add(new Date(log.checkIn).toDateString());
        });

        return Array.from(report.values()).map(entry => {
            const attended = entry.attendedDates.size;
            const percentage = Math.min(100, Math.round((attended / finalExpected) * 100));
            return {
                ...entry,
                attended,
    ...
    const StatusBadge: React.FC<{status: VisitorStatus}> = ({status}) => {
        const styles = {
            ...
            [VisitorStatus.ATTENDED]: 'bg-purple-500/20 text-purple-400',
        };
        return <span className={`px-2.5 py-1 text-xs font-semibold rounded-full ${styles[status]}`}>{status.replace('_', ' ')}</span>;
    };

    const handleDownloadLogs = (logData: VisitorLog[], context: 'gate' | 'intern') => {
        if (logData.length === 0) {
        ...
        const dataToExport = logData.map(log => {
            const base: Record<string, any> = {
                Name: log.name,
                Status: log.status,
                Type: log.visitorType === VisitorType.CORPER ? 'NYSC' : log.visitorType || 'N/A',
            };
            if (context === 'gate') {
                 return {
                    ...base,
                ...
                return {
                    ...base,
                    "Attendance Time": new Date(log.checkIn).toLocaleString(),
                };
            }
        });

        const filename = context === 'gate' ? 'visitor_logs' : 'intern_logs';
        downloadXLSX(dataToExport, `${filename}_${new Date().toISOString().split('T')[0]}`);
    };
    ...
    return (
        <div className="container mx-auto p-4 sm:p-6 lg:p-8">
            ...
            <div className="flex border-b border-gray-700 mb-6 overflow-x-auto">
                <button onClick={() => setView('logs')} className={`px-6 py-3 font-medium flex-shrink-0 ${view === 'logs' ? 'text-emerald-400 border-b-2 border-emerald-400' : 'text-gray-400'}`}>Visitor Logs</button>
                <button onClick={() => setView('interns')} className={`px-6 py-3 font-medium flex-shrink-0 ${view === 'interns' ? 'text-emerald-400 border-b-2 border-emerald-400' : 'text-gray-400'}`}>Interns Attendance</button>
                <button onClick={() => setView('qr')} className={`px-6 py-3 font-medium flex-shrink-0 ${view === 'qr' ? 'text-emerald-400 border-b-2 border-emerald-400' : 'text-gray-400'}`}>QR Management</button>
                <button onClick={() => setView('events')} className={`px-6 py-3 font-medium flex-shrink-0 ${view === 'events' ? 'text-emerald-400 border-b-2 border-emerald-400' : 'text-gray-400'}`}>Event Analytics</button>
            </div>
                        ...
                        <div className="flex items-center gap-4 flex-wrap w-full md:w-auto">
                             {view === 'interns' ? (
                                <div className="flex space-x-2 bg-gray-900 p-1 rounded-md">
                                    <button onClick={() => setInternViewType('daily')} className={`px-4 py-1.5 text-sm font-medium rounded ${internViewType === 'daily' ? 'bg-emerald-600 text-white' : 'text-gray-300 hover:bg-gray-700'}`}>Daily Logs</button>
                                    <button onClick={() => setInternViewType('report')} className={`px-4 py-1.5 text-sm font-medium rounded ${internViewType === 'report' ? 'bg-emerald-600 text-white' : 'text-gray-300 hover:bg-gray-700'}`}>Monthly Report</button>
                                </div>
                             ) : (
                                <div className="flex space-x-2 bg-gray-900 p-1 rounded-md">
                             ...
                             {/* Search Bar - Hide in Report View */}
                             {(view !== 'interns' || internViewType === 'daily') && (
                                <div className="relative">
                                    ...
                                    <input
                                        type="text"
                                        placeholder={`Search ${view === 'logs' ? 'logs' : 'interns'}...`}
                                        value={searchQuery}
                             ...
                             {/* Month/Year Selectors for Report View */}
                             {view === 'interns' && internViewType === 'report' && (
                                 <div className="flex space-x-2">
                                     <select 
                                        value={reportMonth} 
                                        onChange={(e) => setReportMonth(parseInt(e.target.value))}
                                        className="bg-gray-900 border border-gray-700 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
                                     >
                         ...
                         <div className="flex items-center gap-2">
                            ...
                            {(view === 'logs' || (view === 'interns' && internViewType === 'daily')) && (
                                <Button onClick={() => handleDownloadLogs(view === 'logs' ? filteredLogs : filteredInternLogs, view === 'logs' ? 'gate' : 'intern')} disabled={isLoading} title="Download as Excel" className="!px-4 !py-2 bg-gray-600 hover:bg-gray-500 focus:ring-gray-400">
                         ...
                         <table className="w-full min-w-[1000px] text-sm text-left text-gray-300">
                             <thead className="text-xs text-gray-400 uppercase bg-gray-700/50">
                                <tr>
                                    ...
                                    {view === 'interns' && internViewType === 'report' ? (
                                        <>
                                            <th scope="col" className="px-6 py-3">Intern Name</th>
                                            <th scope="col" className="px-6 py-3">Type</th>
                                            <th scope="col" className="px-6 py-3">Days Attended</th>
                                            <th scope="col" className="px-6 py-3">Expected Days (3/week)</th>
                                            <th scope="col" className="px-6 py-3">Attendance Score</th>
                                            <th scope="col" className="px-6 py-3 w-1/3">Progress</th>
                                        </>
                                    ) : (
                                        <>
                                            <th scope="col" className="px-6 py-3">Name</th>
                                            <th scope="col" className="px-6 py-3">Type</th>
                                            {view === 'logs' && <th scope="col" className="px-6 py-3">Department</th>}
                                            {view === 'logs' && <th scope="col" className="px-6 py-3">Laptop Name</th>}
                                            {view === 'logs' && <th scope="col" className="px-6 py-3">Laptop Color</th>}
                                            {view === 'logs' && <th scope="col" className="px-6 py-3">Serial Number</th>}
                                            <th scope="col" className="px-6 py-3">{view === 'interns' ? 'Attendance Time' : 'Check In'}</th>
                                            {view === 'logs' && <th scope="col" className="px-6 py-3">Check Out</th>}
                                            {view === 'logs' && <th scope="col" className="px-6 py-3">Duration</th>}
                                            <th scope="col" className="px-6 py-3 text-center">Status</th>
                                            {view === 'logs' && <th scope="col" className="px-6 py-3 text-center">Actions</th>}
                                        </>
                                    )}
                                </tr>
                            </thead>
                            <tbody>
                                {isLoading ? (
                                    <tr><td colSpan={12} className="text-center p-8"><Spinner /></td></tr>
                                ) : (
                                    view === 'interns' && internViewType === 'report' ? (
                                        internReportData.length > 0 ? (
                                            internReportData.map((data, idx) => (
                                                <tr key={idx} className="bg-gray-800 border-b border-gray-700 hover:bg-gray-700/50">
                                                    <td className="px-6 py-4 font-medium whitespace-nowrap">{data.name}</td>
                                                    <td className="px-6 py-4">{data.type === VisitorType.CORPER ? 'NYSC' : data.type}</td>
                                                    <td className="px-6 py-4 font-bold text-white">{data.attended}</td>
                                                    <td className="px-6 py-4 text-gray-400">{data.expected}</td>
                                                    <td className="px-6 py-4 font-bold">{data.percentage}%</td>
                                                    <td className="px-6 py-4">
                                                        <div className="w-full bg-gray-700 rounded-full h-2.5 dark:bg-gray-700">
                                                            <div 
                                                                className={`h-2.5 rounded-full ${data.percentage >= 75 ? 'bg-emerald-500' : data.percentage >= 50 ? 'bg-yellow-500' : 'bg-red-500'}`} 
                                            ...
                                            <tr><td colSpan={6} className="text-center p-8">No intern attendance data for this month.</td></tr>
                                        )
                                    ) : (
                                        (view === 'logs' ? filteredLogs : filteredInternLogs).length > 0 ? (
                                            (view === 'logs' ? filteredLogs : filteredInternLogs).map(log => (
                                                 <tr key={log.id} className="bg-gray-800 border-b border-gray-700 hover:bg-gray-700/50">
                                                    ...
                                                    <td className="px-6 py-4">{log.visitorType === VisitorType.CORPER ? 'NYSC' : log.visitorType || '—'}</td>
                                                    {view === 'logs' && <>
                                                    ...
                                                    {view === 'logs' && (
                                                        <td className="px-6 py-4 text-center">
                                                            <button 
                                                                ...
                                                                className="p-2 rounded-full text-emerald-400 hover:text-emerald-300 hover:bg-gray-700 transition-colors"
                                                                title="Edit Log"
                                                            >
                                             ...
                                             <tr><td colSpan={view === 'logs' ? 11 : 4} className="text-center p-8">No {view === 'logs' ? 'gate' : 'intern'} logs found.</td></tr>
                                        )
                                    )
                                )}
                            </tbody>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <Card>
                        <div className="p-6">
                            ...
                            <p className="text-gray-400 mb-4">For general visitor check-in/out.</p>
                            <div 
                     ...
                     <Card>
                        <div className="p-6">
                            <h3 className="text-xl font-bold mb-4">Interns Attendance QR</h3>
                            <p className="text-gray-400 mb-4">Refreshes every 5 seconds.</p>
                            <div 
                                className="bg-white p-4 inline-block rounded-lg shadow-md cursor-pointer hover:opacity-80 transition-opacity"
                                onClick={openInternQrModal}
                            >
                                <img key={internQRUrl} src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(internQRUrl)}`} alt="Interns Attendance QR Code" />
                            </div>
                             <Button onClick={() => handleDownloadGenericQR(internQRUrl, 'QR_Intern_Attendance')} className="mt-4"><DownloadIcon className="w-5 h-5 mr-2"/> Download Current</Button>
                        </div>
                    </Card>
                    <Card>
                        <div className="p-6">
                            ...
                            <div className="mb-6 space-y-3">
                                <p className="text-gray-400">Create a unique QR code for a specific event.</p>
            ...
            <Modal isOpen={isGateQrModalOpen} onClose={closeGateQrModal} title="Main Gate QR Code">
                <div className="flex flex-col items-center text-center">
                    ...
                    <Button onClick={() => handleDownloadGenericQR(gateQRUrl, 'QR_Main_Gate')}>
                        <DownloadIcon className="w-5 h-5 mr-2" />
                        Download QR Code
                    </Button>
                </div>
            </Modal>
            <Modal isOpen={isInternQrModalOpen} onClose={closeInternQrModal} title="Interns Attendance QR">
                <div className="flex flex-col items-center text-center">
                    <p className="text-gray-400 mb-4">Scan for intern attendance. Code refreshes automatically.</p>
                    <div className="bg-white p-4 inline-block rounded-lg shadow-md mb-6">
                        <img 
                            key={internQRUrl} 
                            src={`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(internQRUrl)}`} 
                            alt="Interns Attendance QR Code" 
                        />
                    </div>
                    <Button onClick={() => handleDownloadGenericQR(internQRUrl, 'QR_Intern_Attendance')}>
                        <DownloadIcon className="w-5 h-5 mr-2" />
                        Download QR Code
                    </Button>
                </div>
            ...
            <Modal isOpen={isManualEntryModalOpen} onClose={() => setIsManualEntryModalOpen(false)} title="Add Manual Visitor Log" size="lg">
                <form onSubmit={handleManualEntrySubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                         <div>
                            <label htmlFor="manual-laptopName" className="block text-sm font-medium text-gray-300 mb-1">Laptop Name <span className="text-red-400">*</span></label>
                            <Input id="manual-laptopName" name="laptopName" placeholder="e.g., HP EliteBook" value={manualEntryData.laptopName} onChange={handleManualEntryChange} required />
                        </div>
                        <div>
                           <label htmlFor="manual-laptopColor" className="block text-sm font-medium text-gray-300 mb-1">Laptop Colour <span className="text-red-400">*</span></label>
                            <Input id="manual-laptopColor" name="laptopColor" placeholder="e.g., Silver" value={manualEntryData.laptopColor} onChange={handleManualEntryChange} required />
                        </div>
                         <div>
                           <label htmlFor="manual-serialNumber" className="block text-sm font-medium text-gray-300 mb-1">Serial Number <span className="text-red-400">*</span></label>
                            <Input id="manual-serialNumber" name="serialNumber" placeholder="Serial Number" value={manualEntryData.serialNumber} onChange={handleManualEntryChange} required />
                        </div>
                    </div>
            ...
            <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="Edit Visitor Log" size="lg">
                <form onSubmit={handleUpdateLogSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label htmlFor="edit-name" className="block text-sm font-medium text-gray-300 mb-1">Full Name <span className="text-red-400">*</span></label>
                            <Input id="edit-name" name="name" placeholder="Full Name" value={editFormData.name} onChange={handleEditFormChange} required />
                        </div>
                        <div>
                            <label htmlFor="edit-department" className="block text-sm font-medium text-gray-300 mb-1">Department <span className="text-red-400">*</span></label>
                            <Input id="edit-department" name="department" placeholder="Department" value={editFormData.department} onChange={handleEditFormChange} required />
                        </div>
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                         <div>
                            <label htmlFor="edit-laptopName" className="block text-sm font-medium text-gray-300 mb-1">Laptop Name <span className="text-red-400">*</span></label>
                            <Input id="edit-laptopName" name="laptopName" placeholder="e.g., HP EliteBook" value={editFormData.laptopName} onChange={handleEditFormChange} required />
                        </div>
                        <div>
                           <label htmlFor="edit-laptopColor" className="block text-sm font-medium text-gray-300 mb-1">Laptop Colour <span className="text-red-400">*</span></label>
                            <Input id="edit-laptopColor" name="laptopColor" placeholder="e.g., Silver" value={editFormData.laptopColor} onChange={handleEditFormChange} required />
                        </div>
                         <div>
                           <label htmlFor="edit-serialNumber" className="block text-sm font-medium text-gray-300 mb-1">Serial Number <span className="text-red-400">*</span></label>
                            <Input id="edit-serialNumber" name="serialNumber" placeholder="Serial Number" value={editFormData.serialNumber} onChange={handleEditFormChange} required />
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-x-6 gap-y-2 pt-1">
                            {Object.values(VisitorType).map((type) => (
                                <div key={type} className="flex items-center">
                                    <input
                                        id={`edit-visitorType-${type}`}
                                        name="visitorType"
                                        type="radio"
                                        value={type}
                                        checked={editFormData.visitorType === type}
                                        onChange={handleEditFormChange}
                                        required
                                        className="h-4 w-4 text-emerald-600 bg-gray-700 border-gray-600 focus:ring-emerald-500"
                                    />
                                    <label htmlFor={`edit-visitorType-${type}`} className="ml-2 block text-sm text-gray-300">
                                        {type}
                                    </label>
                                </div>
                            ))}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-md text-gray-300">
                                {editingLog ? new Date(editingLog.checkIn).toLocaleString() : ''}
                            </p>
                        </div>
                         <div>
                            <label className="block text-sm font-medium text-gray-400 mb-1">Check-out Time</label>
                            <p className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-md text-gray-300">
                                {editingLog?.checkOut ? new Date(editingLog.checkOut).toLocaleString() : 'Not checked out'}
                            </p>
                        </div>
                    </div>

                    <div className="flex justify-end gap-4 pt-4">
                        <Button type="button" onClick={() => setIsEditModalOpen(false)} className="bg-gray-600 hover:bg-gray-700 focus:ring-gray-500">
                            Cancel
                        </Button>
                        <Button type="submit" disabled={isSubmittingEdit}>
                            {isSubmittingEdit ? <Spinner /> : 'Save Changes'}
                        </Button>
                    </div>
                </form>
            </Modal>
